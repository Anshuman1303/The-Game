...
"""
cmdlist=["cmd","register","login","logout","msg","inbox","history","send","gamble","horserace","work","display","update","ban","unban","hangman"]
    ...
    if cmd.split()[0].lower()=="work":          #Work
        ...
        while True:
            ...
            if uans.isnumeric():
                ...
                display("Enter a valid number")
    if cmd.split()[0].lower()=="hangman":       #hangman
        hangman='_________________\n  |_|\t |\n  |_|\t |\n  |_|    O\n  |_|\t/|\ \n  |_|\t/ \ \n  |_|__ '
        words=["come","bear","play","sing","bean","game","rice","four","five","made","laid","dogs","cats","hats","card","fade","fail","iced","moon"]
        word=random.choice(words)
        characters=list(word)
        character=random.choice(characters)
        characterindex=characters.index(character)    
        display("Here's the 4-letter word you need to find! Guess a letter you think might be in this word")
        dispword=""
        for i in characters:
            if i==character:
                dispword+=character
        ...
        alrguessed=[]
        while win==False:
            display(f"Word: {dispword}")
            display("Take your guess!")
            guess=input().lower()
            if len(guess)>1:
                display("You're supposed to guess a letter")
            elif guess==character:
                display("That's already visible to you.")
            elif guess in alrguessed:
                display("You already guessed this letter.")
            elif guess not in alrguessed and guess!=character:
                if guess in characters:
                    alrguessed.append(guess)
                    display("GOOD GUESS! That's in the word\nI'll add it to the word.")
                    guessindex=characters.index(guess)
                    displist=list(dispword)
                    for i in range(0,len(characters)):
                        if characters[i]==guess:
                            displist[i]=characters[i]
                    dispword=""
                    for i in displist:
                        dispword+=i
                    if not "_" in displist:
                        user[2]+=50
                        update()
                        display(f"You won $50, congratulations!\n The word was {dispword}")
                        display(f"Now you have ${user[2]}")
                        win=True
                if guess not in characters:
                    alrguessed.append(guess)
                    count+=1
                    display("Nope! Not in the word. Let's take a look at hangman.")
                    hangmancount(count)
                    if count==6:
                        display(f"DAMN, you lost the game The word was {word}.\nBetter luck next time.")
                        break
